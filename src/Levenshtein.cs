...
public class Levenshtein
{
    ...
    {
        Argument<FileInfo> dictionaryArg = new Argument<FileInfo>("dictionary",
            "The path to a file containing the list of words in the dictionary");
        Argument<DatabaseType> databaseArg = new Argument<DatabaseType>(name: "database",
            description: "The type of database algorithm", getDefaultValue: () => DatabaseType.cache);
        Argument<FinderAlgorithm> finderArg = new Argument<FinderAlgorithm>(name: "finder",
            description: "The path finder algorithm", getDefaultValue: () => FinderAlgorithm.dual);

        Option<FileInfo> wildcardMapOpt = new Option<FileInfo>("--card-map",
            "The path to a file containing the wildcard map. Only applicable if 'cache' is the selected database");
        ...
        Command analyzeVerb =
            new Command("analyze", "Analyze all paths between each pair of words in the entire dictionary")
        ...
        Option<uint> repsOpt = new Option<uint>(name: "--reps",
            description:
            "How many times to repeat each word pair search. Higher gives more accurate data but takes longer",
        ...
        pathVerb.SetHandler(Levenshtein.RunPath, dictionaryArg, wordArg1, wordArg2, databaseArg, finderArg,
            wildcardMapOpt);

        Argument<FileInfo> wildcardDestinationArg =
            new Argument<FileInfo>(name: "destination", description: "File to cache map to");
        ...
        RootCommand rootCommand =
            new RootCommand("CLI for the Levenshtein graph problem. See https://github.com/TristenYim/Levenshtein")
                { testVerb, benchmarkVerb, pathVerb, analyzeVerb, cacheVerb };

        return rootCommand.Invoke(args);
    }

    private static void RunPath(FileInfo dictionaryPath, String word1, String word2, DatabaseType databaseType,
        FinderAlgorithm finderAlgorithm, FileInfo wildcardPath)
    {
    ...
    private static void RunTest(FileInfo dictionaryPath, DatabaseType databaseType, FinderAlgorithm finderAlgorithm,
        FileInfo wildcardPath)
    {
        ...
        Console.WriteLine("Total time (excluding init) is " + (Stopwatch.GetTimestamp() - time1) / ticksPerMs +
                          " milliseconds");
    }

    private static void RunBenchmark(FileInfo dictionaryPath, DatabaseType databaseType,
        FinderAlgorithm finderAlgorithm, FileInfo wildcardPath, uint reps)
    {
        long time0 = Stopwatch.GetTimestamp();
        ...
        foreach (String[] wordPair in _benchmarkWords)
        {
            ...
            {
                LevenshteinPathFinder.PathsToString(finder.GeneratePaths(wordIndex1, wordIndex2, database), database,
                    false, false);
            }

            Console.WriteLine("Average time is " + (((Stopwatch.GetTimestamp() - time2) / ticksPerMus) / reps) +
                              " microseconds\n");
        }
    }
    ...
    private static void RunAnalyze(FileInfo dictionaryPath, DatabaseType databaseType, FileInfo wildcardPath)
    {
        ...
        Console.WriteLine("Done! Time to complete was " + (Stopwatch.GetTimestamp() - time1) / ticksPerMs +
                          " milliseconds");
    }
    ...
    private static LevenshteinDatabase GetDatabase(FileInfo dictionaryPath, DatabaseType databaseType,
        FileInfo wildcardPath)
    {
        switch (databaseType)
        {
    ...
    private static void WordPairTest(String word1, String word2, LevenshteinPathFinder finder,
